...
exports.replaceText = function (settings) {
  ...
  return gruntTextReplace.searchTextMultiple(text, replacements);
}

exports.replaceFile = function (settings) {
  return gruntTextReplace.searchFile(settings)
}

exports.replaceFileMultiple = function (settings) {
  return gruntTextReplace.searchFileMultiple(settings)
}
...
gruntTextReplace = {
  searchFileMultiple: function (settings) {
    ...
    if (isReplacementRequired) {
      grunt.file.copy(pathToSourceFile, pathToDestinationFile, {
        process: function (text) {
          return gruntTextReplace.searchTextMultiple(text, replacements, pathToSourceFile);
        }
      });
    }
  },

  searchTextMultiple: function (text, replacements, pathToSourceFile) {
    return replacements.reduce(function (newText, replacement) {
      return gruntTextReplace.searchText({
        text: newText,
  ...
  searchText: function (settings, pathToSourceFile) {
    var text = settings.text;
  ...
  match: function (settings) {
    ...
    exclusionFileData = JSON.parse(fs.readFileSync(settings.exclusionFile));
    ...
    if (typeof dest === 'undefined' &&
      ...
      gruntTextReplace.searchFileMultiple({
        src: src,
        dest: dest,
  ...
  reportMatch: function (matchedSubstring, index, fullText, regexMatches, pathToSourceFile, translationAttribute) {
     var sourceString = matchedSubstring.replace(/(<([^>]+)>)/ig, "");
      ...
      if (sourceString) {
        ...
        var exclusion = false;
         for (e = 0; e < exclusionFileData.exclusions.length; e++){

          //if sourceString contains a string in the exclusionFileData
          //that is to be excluded then check the pathToSourceFile to see
          //if it is contained in the excludePaths, if so it is an exclusion
          if(sourceString.indexOf(exclusionFileData.exclusions[e].string) > -1){

             //check to see if there is a path in the excludePaths that
             //matches the pathToSourceFile, if so this is an exclusion
             //and should not be logged.
              exclusion = exclusionFileData.exclusions[e]
                                           .excludePaths
                                           .some(function(path){
                                                                return path === pathToSourceFile || path === "*";
                                                               });
          }
        }
